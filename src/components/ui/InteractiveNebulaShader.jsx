...
export function InteractiveNebulaShader({
  ...
  useEffect(() => {
    ...
    renderer.setClearColor(0x000000, 0); // Transparent lead
      ...
      void mainImage(out vec4 O, in vec2 fragCoord) {
        ...
        for (int i = 0; i <= 5; i++) {
          ...
          vec3 base = hasActiveReminders
            ? vec3(0.05,0.2,0.5) + vec3(4.0,2.0,5.0)*f // Electric Blue
            : hasUpcomingReminders
            ? vec3(0.2,0.05,0.4) + vec3(3.0,1.0,4.0)*f // Deep Purple
            : vec3(0.1,0.3,0.4) + vec3(5.0,2.5,3.0)*f; // Cyan/Vibrant Sky

          col = col * base + smoothstep(2.5, 0.0, rz) * 0.7 * base;
          d += min(rz, 1.0);
        }

        // No center dimming for maximum visibility
        O = vec4(col, 1.0);
      }
  ...
  return (
    ...
    >
      {/* Metallic Geometric Design Overlay */}
